...
We could then use our `addBirthCountry()` function as part of a larger query to summarize the proportion of players from each country over time (based on their presence in the batting table).
...
Following on from the above approach, we might think it a good idea to make another function `addBirthYear()`. We can then use it along with our `addBirthCountry()` to get a summarized average salary by birth country and birth year.
...
Let's say we want to summarize home runs in the batting table and strike outs in the pitching table by the college players attended and their birth year. We could do this like so:
...
To avoid this we could instead make use of the [`compute()`](https://dplyr.tidyverse.org/reference/compute.html) function to force the computation of this first, intermediate, query to a temporary table in the database.
