...
![`nycflights13` relational diagram from <https://collinn.github.io/teaching/2023/labs/joins.html>.](../images/relational-01.png){.lightbox #fig-nycflights13}
...
::: callout-important
Until we use `compute()` or `collect()` (or printing the first few rows of the result) all we're doing is translating R code into SQL. Which means no code is being executed in the database side.
...
-   printing (e.g. `glimpse()` or `print()`) will execute the query limiting the result to the first set of rows which leads to smaller computation time in the database side.
:::
