...
In the previous chapter @sec-dbplyr_verbs we saw that there are a core set of tidyverse functions that can be used with databases to extact data for analysis. The SQL code used in the previous chapter would be the same for all database management systems, with only joins and variable selection being used.
