# Getting started with R and database
...
Or we could take a look at the first row of the data using `head()`
...
Let's put our penguins data into a duckdb database. We first create the duckdb database, then add the penguins data, and the create a reference to the table with the data.
...
Now the data is in a database we could use SQL to get the first row that we saw before

```{r}
dbGetQuery(db, "SELECT * FROM penguins LIMIT 1")
```
...
## Organising data analyses 

![](images/EnepKxNVQAAhNhg.jpg){width="250"}

*Artwork by \@allison_horst*

## Reproducibility in renv
