...
For this chapter, we'll use a synthetic COVID-19 dataset (`"synthea-covid19-10k"`). A characterisation of this dataset can be found [here](https://dpa-pde-oxford.shinyapps.io/OmopSketchCharacterisation/).
...
By default the mock cdm returns a local data set that can be inserted in any source of interest using the `insertCdmTo()` function. Populating source with duckdb will return you an in memory duckdb cdm_reference with two schemas cdmSchema = "main" and writeSchema = "results".
:::
