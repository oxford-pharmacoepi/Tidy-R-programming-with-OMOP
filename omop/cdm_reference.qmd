...
As we saw in @sec-data_model, creating a data model in R to represent the OMOP CDM can provide a basis for analytic pipelines using the data. Luckily for us, we won't have to create functions and methods for this ourselves. Instead, we will use the [`omopgenerics`](https://darwin-eu.github.io/omopgenerics) package which defines a data model for OMOP CDM data and the [`CDMConnector`](https://darwin-eu.github.io/CDMConnector) package which provides functions for connecting to OMOP CDM data held in a database.

To see how this works, we will use the [`omock`](https://ohdsi.github.io/omock) package to create example data in the format of the OMOP CDM, which we will then copy to a DuckDB database created by the [`duckdb`](https://r.duckdb.org) package.
...
library(omopgenerics) # to be removed after CDMConnector > 2.2.0
...
Note that here we have first created a local version of the cdm with all the tables of interest with [`omock`](https://ohdsi.github.io/omock/) (`cdm_local`), then copied it to a DuckDB database, and finally created a reference to it with [`CDMConnector`](https://darwin-eu.github.io/CDMConnector), so that we can work with the final `cdm` object as we normally would for one created with our own healthcare data. In that case, we would directly use [`cdmFromCon()`](https://darwin-eu.github.io/CDMConnector/reference/cdmFromCon.html) with our own database information. Throughout this chapter, however, we will keep working with the mock dataset.

## CDM attributes

### CDM name

Our cdm reference will be associated with a name. By default, this name will be taken from the `cdm_source_name` field from the `cdm_source` table. We will use the function [`cdmName()`](https://darwin-eu.github.io/omopgenerics/reference/cdmName.html) from [`omopgenerics`](https://darwin-eu.github.io/omopgenerics/) to get it.
...
We can also easily check the OMOP CDM version that is being used with the function [`cdmVersion()`](https://darwin-eu.github.io/omopgenerics/reference/cdmVersion.html) from [`omopgenerics`](https://darwin-eu.github.io/omopgenerics) like so:
...
Although as stated, the [`cdmName()`](https://darwin-eu.github.io/omopgenerics/reference/cdmName.html) and [`cdmVersion()`](https://darwin-eu.github.io/omopgenerics/reference/cdmVersion.html) functions are defined by the omopgenerics packages, these functions are re-exported in other packages and you won't need to load omopgenerics explicitly.
:::
...
In general the source is only used internally and it is an object defined by the connecting package, in this case [CDMConnector](https://darwin-eu.github.io/CDMConnector/) and allows that all the different functions such as `listSourceTables()`, `dropSourceTable()`, `readSourceTable()` and `compute()` (that we will see later in detail) work correctly in the source backend.

::: {.callout-note collapse="true"}
### Explore the source object

In the CDMConnector case the source object contains the connection and the writting schema:
...
By default, only the default omop tables [`omopTables()`](https://darwin-eu.github.io/omopgenerics/reference/omopTables.html) will be included (if they exist) into the cdm_reference object.
:::

## Including achilles tables in the cdm reference

If we have the results tables from the [`Achilles`](https://ohdsi.github.io/Achilles/) package in our database, we can also include these in our cdm reference.
...
Note we specified the `achillesSchema` that in this case is the same as the `writeSchema` and `cdmSchema`, but each one of them can be different and point to a separate [schema](https://www.geeksforgeeks.org/dbms/database-schemas/) in our database.

## Adding other tables to the cdm reference

Let's say we have some additional **local** data that we want to add to our cdm reference. We can add this both to the same source (in this case a database) and to our cdm reference using [`insertTable()`](https://darwin-eu.github.io/omopgenerics/reference/insertTable.html) from [`omopgenerics`](https://darwin-eu.github.io/omopgenerics/) ([`insertTable()`](https://darwin-eu.github.io/omopgenerics/reference/insertTable.html) is also re-exported in [`CDMConnector`](https://darwin-eu.github.io/CDMConnector/)). We will show this with the dataset `cars` built-in to R.
...
Once we have this table in the database, we can just read it using the [`readSourceTable()`](https://darwin-eu.github.io/omopgenerics/reference/readSourceTable.html) function.
...
Note that omopgenerics provides these functions [`readSourceTable()`](https://darwin-eu.github.io/omopgenerics/reference/readSourceTable.html), [`listSourceTables()`](https://darwin-eu.github.io/omopgenerics/reference/listSourceTables.html), and [`dropSourceTable()`](https://darwin-eu.github.io/omopgenerics/reference/dropSourceTable.html) for the easier management of the tables in the writeSchema.
