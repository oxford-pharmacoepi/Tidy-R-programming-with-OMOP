...
To see how this works we will use the [`omock`](omock.github.io/omopgenerics) package to create example data in the format of the OMOP CDM, which we will then copy to a DuckDB database created by the [`duckdb`](https://r.duckdb.org) package.
...
By default only the default omop tables [`omopTables()`](darwin-eu.github.io/omopganerics/reference/omopTables.html) will be included (if they exists) into the cdm_reference object.
:::
...
Note that omopgenerics provide these functions [`readSourceTable()`](darwin-eu.github.io/omopgenerics/reference/readSourceTable.html), [`listSourceTables()`](darwin-eu.github.io/omopgenerics/reference/listSourceTables.html), and [`dropSourcTables()`](darwin-eu.github.io/omopgenerics/reference/dropSourcTables.html) for the easier management of the tables in the writeSchema.
