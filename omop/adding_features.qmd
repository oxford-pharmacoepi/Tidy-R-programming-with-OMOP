...
As we have seen, the age groups have been named using their lower and upper bounds ('0 to 17', '18 to 64' and '65 or above'). But we can customize those labels by naming the age group:
...
If you take a close look at the documentation of the function, you will see that you can also add multiple age groups and control the name of the new column that, as we have seen, by default is 'age_group'.
:::
...
In the first case, where we want to add a new variable derived from other variables in our table, we'll typically be using [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) (from the [`dplyr`](https://dplyr.tidyverse.org) package). For example, perhaps we just want to add a new variable to our observation period table containing the year of individuals' observation period start dates. This is rather straightforward.
