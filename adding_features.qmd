...
```{r}
covid_records <- cdm$condition_occurrence |>
  filter(condition_concept_id == 37311061)

covid_records <- covid_records |> 
  addSex() |> 
...
Note `addAge()` and `addSex()` will add the patients demographics and compute it in a temporary table (use `name` argument if you want it to be a permanent table). If instead of creating the temporary table we want just to add the query we can use the same functions ended in `Query` (e.g. `addAgeQuery()` or `addSexQuery()`).

See that now `covid_records` is the reference to a table as we used the `addAge()` and `addSex` function.
...
whereas if we use the `addSexQuery` function we are 'just' adding the query as we can see here
...
ggplot(covid_records, aes(fill = sex)) +
  facet_grid(sex ~ .) +
